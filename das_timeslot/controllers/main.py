...
class TimeSlot(http.Controller):
    ...
    @http.route('/api/time-slot', type='json', auth='public', methods=['Post'], cors="*")
    def get_time_slot(self):
        ...
        dayofweek = req.get('dayofweek')
        is_tomorrow = req.get('is_tomorrow')
        list_products = req.get('list_products')
        is_delivery = req.get('is_delivery')
        ...
        if calendar:
            calendar_attendance = request.env['resource.calendar.attendance'].sudo().search(
                [('calendar_id', '=', calendar[0].id), ('dayofweek', '=', dayofweek)], order='hour_from ASC')
            ...
            if order_ok == False:
                ...
                return response
            max_preparation_time = self.get_max_preparation_time(list_products)
