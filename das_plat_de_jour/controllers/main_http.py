...
class PlatDeJoue(http.Controller):
    @http.route('/api/plat-du-jour-http', type='http', auth="public", cors='*', methods=['Get'])
    def get_plat_de_jour(self):
        base_url = request.env['ir.config_parameter'].sudo().get_param('web.base.url')
        if  ('https' in base_url) == False:
            base_url = base_url.replace('http', 'https')
        try:
            day_of_week = int(request.params.get('day_of_week'))
        except:
            day_of_week = -1
        ...
        if day_of_week!=-1:
            ...
            if plat:
                ...
                response = {'status': 200,'response': [],'message': 'No plat Found'}
        else:
    ...
    @http.route('/api/plat-du-jour-all-http', type='http', auth="public", cors='*', methods=['Get'])
    def get_plat_de_jour_all(self):
        ...
        if plats:
            ...
            # sorted_data_descending = sorted(vals_list, key=lambda x: x["day_of_week"], reverse=True)
