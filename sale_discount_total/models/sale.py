...
class SaleOrder(models.Model):
    ...
    def write(self, vals):
        if 'discount_rate' in vals:
            if vals['discount_rate']>100.0:
                raise ValidationError(_("discount rate must be less than 100"))
            if vals['discount_rate']<0.0:
                raise ValidationError(_("discount rate must be great than 0"))
